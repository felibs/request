var RequestHooks = (function (exports, vue, axios) {
    "use strict";
    ...
    ***************************************************************************** */
    ...
    function isPromise(fn) {
        return (
            type(fn) === "Promise" ||
            (type(fn) === "Function" &&
                type(fn.then) === "Function" &&
                type(fn.catch) === "Function")
        );
    }
    // 最先执行的format函数
    ...
    function serializeOption(option) {
        ...
        return {
            ...
            refreshDeps: Array.isArray(option.refreshDeps)
                ? option.refreshDeps
                : null,
        };
    }
    function mergeArgs(methods, option, args) {
        return methods.map(function (item) {
            ...
            if (item.type === execItemType.string) {
                ...
                item.args[0].params = Object.assign(
                    {},
                    option.params,
                    runParams
                );
                ...
                (_a = item.args).push.apply(
                    _a,
                    __spread(toArray(args), toArray(option.params))
                );
            } else if (item.type === execItemType.axios) {
                if (typeof args === "object" && args !== null) {
                    runParams = args;
                }
                var method = item.args[0].method
                    ? item.args[0].method.toLocaleUpperCase()
                    : "GET";
                // todo 多种方法参数的支持
                if (method === "GET") {
                    item.args[0].params = Object.assign(
                        {},
                        option.params,
                        item.args[0].params ? item.args[0].params : {},
                        runParams
                    );
                    item.args[0].method = "GET";
                } else if (method === "POST") {
                    item.args[0].data = Object.assign(
                        {},
                        option.params,
                        item.args[0].data ? item.args[0].data : {},
                        runParams
                    );
                }
            }
    ...
    function asyncExecuter(fnList) {
        return new Promise(function (resolve) {
            asyncEach(
                fnList,
                function next(item, callbck) {
                    execute(item.fn, item.args).then(function (_a) {
                        var data = _a.data,
                            error = _a.error;
                        callbck(error, data);
                    });
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        return [
                            4 /*yield*/,
                            execute(execItem.fn, execItem.args),
                        ];
                    case 2:
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [3 /*break*/, 4];
                    case 2:
                        return [4 /*yield*/, asynchronousExecuter(fnList)];
                    case 3:
                        result = _a.sent();
                        _a.label = 4;
                    case 4:
    ...
    function useRequest(method, options) {
        ...
        var setLoading = function (status, key) {
            return __awaiter(_this, void 0, void 0, function () {
                var fetcheState;
                return __generator(this, function (_a) {
                    if (serializedOptions.key) {
                        if (key) {
                            fetcheState = state.fetches[key] || {
                                loading: false,
                                data: null,
                                error: null,
                            };
                ...
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            ...
                            return [
                                4 /*yield*/,
                                execute(serializedOptions.key, toArray(arg)),
                            ]; // 或者key
                        case 1:
                            (_a = _b.sent()), // 或者key
                                (data = _a.data),
        ...
        var setResult = function (result, key) {
            return __awaiter(_this, void 0, void 0, function () {
                    ...
                    switch (_a.label) {
                        case 0:
                            ...
                            return [
                                4 /*yield*/,
                                executer(
                                    serializedOptions.format,
                                    execType.asynchronous
                                ),
                                // 如果存在key，重置数据
                            ]; // 对结果进行format
                        case 1:
                            ...
                            if (serializedOptions.key) {
                                ...
                                if (serializedOptions.cacheKey) {
                                    cache[serializedOptions.cacheKey] =
                                        state.data;
                                }
                            }
                            // 执行回掉函数
        ...
        if (serializedOptions.manual === false) {
            ...
            Object.assign(config, {
                run: wrapperRun(serializedOptions, run),
                cancel: cancel,
            });
        }
        if (
            serializedOptions.refreshDeps &&
            serializedOptions.manual === false
        ) {
